...
async function main() {
  ...
  console.log(`⏱️  时间范围: 近${CONFIG.days}天`);
  ...
  try {
    ...
    // 访问招标公告页面
    const listUrl = `${site.url}/xxhz/014001/014001001/transaction_detail.html`;
    console.log(`\n📍 访问列表页: ${listUrl}`);
    ...
    await page.screenshot({ path: path.join(CONFIG.output, 'logs', 'listpage.png'), fullPage: true });
        ...
        if (items.length > 0) {
          ...
          listItems = await page.$$eval(selector, (els) => {
            return els.map(el => {
              ...
              // 提取金额
              const amountMatch = dateText.match(/(\d+\.?\d*)\s*(万元|元|万)/);
              const amount = amountMatch ? amountMatch[0] : '';
              
              return { title, link, date, amount };
    ...
    for (let i = 0; i < toVisit.length; i++) {
      ...
      console.log(`[${i + 1}/${toVisit.length}] 访问: ${item.title.substring(0, 50)}...`);
      ...
      try {
        ...
        const detail = await page.evaluate(() => {
          ...
          // 提取日期
          const dateMatch = amountText.match(/(\d{4}-\d{2}-\d{2})/g);
          const date = dateMatch && dateMatch.length > 0 ? dateMatch[0] : '';
          
          // 提取联系方式
          const contactMatch = amountText.match(/联系电话[：:]\s*([\d\-]+)/);
          const contact = contactMatch ? contactMatch[1] : '';
          
          // 提取招标人
          const bidderMatch = amountText.match(/招标人[：:]\s*([^\n]+)/);
          const bidder = bidderMatch ? bidderMatch[1].trim() : '';
          
          // 提取截止时间
          const deadlineMatch = amountText.match(/截止时间[：:]\s*([^\n]+)/);
        ...
        console.log(`✅ 提取成功: ${detail.title.substring(0, 30)}...`);
